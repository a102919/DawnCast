...
純 Python mock 測試，不連 DB / 不打外部 API：
  - build_daily_messages 產 5 個完整 generate contract
...
DB function 本身（marker claim + 5 send 原子性）需連 DB 的整合測試留給
...
def test_build_daily_messages_produces_5_with_required_fields() -> None:
    """5 個 slot，每個欄位齊全。"""
    messages = daily_batch.build_daily_messages("2026-07-25")

    assert len(messages) == 5

    expected_topics = ["tech", "business", "culture", "science", "tech"]
    expected_canonicals = [
        "AI agents at work",
    ...
    expected_angles = ["定義", "應用場景", "歷史", "常見誤解", "對比"]
...
def test_build_daily_messages_5th_slot_differs_from_1st() -> None:
    """第 5 slot 不能跟第 1 slot 撞 idempotency_key；驗證 angle 不同。"""
    messages = daily_batch.build_daily_messages("2026-07-25")
    assert messages[0]["big_topic"] == messages[4]["big_topic"]  # 都是 tech
    assert messages[0]["angle"] != messages[4]["angle"]  # 但 angle 不同


async def test_enqueue_daily_batch_calls_send_daily_batch(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """happy path：build 完 5 筆 → 呼叫 SQL function → 回傳 5。"""
...
async def test_enqueue_daily_batch_returns_unexpected_count_silently(
    ...
    """DB function 回傳 1~4 不該發生；函式仍回傳該值（不 raise），由 worker log 標紅。"""
    async def fake_send_daily_batch(deliver_date: str, bodies: list[dict[str, Any]]) -> int:
        return 3
