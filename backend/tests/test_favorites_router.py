...
class FakeCursor(_BaseFakeCursor):
    async def execute(self, sql: str, params: tuple[Any, ...] = ()) -> None:
        ...
        if "delete from public.user_favorites" in s:
            ...
            FAVORITES[:] = [(u, e) for u, e in FAVORITES if not (u == user_id and e == ep_uuid)]
            self._rows = []
            return
...
def test_list_favorites_returns_user_slugs(client: TestClient) -> None:
    res = client.get("/favorites", headers=auth_header(USER_A))
...
def test_list_favorites_scoped_to_owner(client: TestClient) -> None:
    res_a = client.get("/favorites", headers=auth_header(USER_A))
    res_b = client.get("/favorites", headers=auth_header(USER_B))
...
def test_remove_favorite_scoped_to_owner(client: TestClient) -> None:
    ...
    client.delete("/favorites/ep-b", headers=auth_header(USER_A))
    res_b = client.get("/favorites", headers=auth_header(USER_B))
